  ...
  const loadOrderData = async () => {
    try {
      ...
      if (response.data.success) {
        // ✅ ADD THIS FILTER: Remove failed Razorpay payments
        const filteredOrders = (response.data.orders || []).filter(order => {
          // Only hide temporary Razorpay orders that are still in 'Payment Pending'
          // Keep cancelled/refunded/replaced orders visible to the user
          if (order.paymentMethod === 'Razorpay' && order.payment !== true && order.status === 'Payment Pending') {
            return false;
          }
          return true;
        });
                ...
                <div className="w-full">
                  ...
                  <div className="text-slate-700 space-y-1 text-xs sm:text-sm">
                    ...
                    {(() => {
                      ...
                      if (order.status === 'Delivered') statusClass = 'text-blue-600';
                      else if (order.status === 'Cancelled') statusClass = 'text-red-400';
                      else if (order.status === 'Cancel and Refund') statusClass = 'text-red-600';
                      else if (order.status === 'Replaced') statusClass = 'text-green-600';

                      // Determine payment text and class
                      let paymentText = 'Pending';
                      let paymentClass = 'text-red-400';

                      if (order.status === 'Cancel and Refund') {
                        paymentText = 'Refunded';
                        paymentClass = 'text-red-600';
                      } else if (order.status === 'Replaced') {
                        paymentText = 'Paid';
                        paymentClass = 'text-blue-600';
                      } else if (order.payment === true || order.status === 'Delivered') {
                        paymentText = 'Paid';
                        paymentClass = 'text-blue-600';
                      }
                      ...
                      return (
                        <>
                          <p>🔄 Status: <span className={`font-semibold ${statusClass}`}>{order.status}</span></p>
                          <p>💳 Payment: <span className={paymentClass}>{paymentText}</span></p>
                        </>
                      );
                    })()}
                    <p>📦 Method: {order.paymentMethod}</p>
