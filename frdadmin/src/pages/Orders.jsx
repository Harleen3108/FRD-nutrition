...
const Orders = ({ token }) => {
  ...
  // ✅ Get payment status for order
  const getPaymentStatus = (order) => {
    // If admin marked as Cancel and Refund -> refunded
    ...
    // Replaced should be displayed as paid
    if (order.status === 'Replaced') {
      return 'paid';
    }

    // Delivered or captured Razorpay payments
    if (order.status === 'Delivered' || (order.paymentMethod === 'Razorpay' && order.payment === true)) {
      return 'paid';
    }
  ...
  // ✅ Search functionality (include per-user order number `orderNo` like `#1`)
  const searchOrders = (orders, searchTerm) => {
    if (!searchTerm.trim()) return orders;
    
    const searchLower = searchTerm.toLowerCase();
    return orders.filter(order => {
      const orderNoStr = order.orderNo ? `#${order.orderNo}` : '';
      return (
        order._id.toLowerCase().includes(searchLower) ||
        orderNoStr.toLowerCase().includes(searchLower) ||
        order.address.firstName.toLowerCase().includes(searchLower) ||
        order.address.lastName.toLowerCase().includes(searchLower) ||
        order.address.email.toLowerCase().includes(searchLower) ||
        order.address.phone.includes(searchTerm) ||
...
const filteredOrders = searchOrders(sortedOrders, searchTerm).filter((order) => {
  ...
  // ✅ Only hide temporary Razorpay orders that are still pending payment
  // Don't hide orders that were cancelled/refunded (they should remain visible)
        ...
        {filteredOrders.map((order, index) => {
          ...
          return (
            ...
            >
              ...
              <div className='text-gray-700 text-xs sm:text-sm space-y-1'>
                <p><span className='font-medium'>Order ID:</span> {order.orderNo ? `#${order.orderNo}` : order._id}</p>
                <p><span className='font-medium'>Items:</span> {order.items.length}</p>
                <p><span className='font-medium'>Method:</span> {order.paymentMethod}</p>
                <p>
                  <span className='font-medium'>Payment:</span> 
