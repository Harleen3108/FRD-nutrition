...
//               {/* Reviews (Admin) */}
//               <div className="mt-4">
//                 <p className="text-sm font-medium text-gray-700">Product Reviews</p>
//                 <div className="mt-2 max-h-48 overflow-y-auto border rounded p-2 bg-gray-50">
//                   {selectedProduct.reviews && selectedProduct.reviews.length > 0 ? (
//                     selectedProduct.reviews.slice().reverse().map((r) => (
//                       <div key={r._id} className="flex items-start justify-between gap-2 p-2 border-b">
//                         <div>
//                           <div className="font-medium">{r.name} <span className="text-xs text-gray-400">({r.rating}/5)</span></div>
...
//                             onClick={async () => {
//                               try {
//                                 const res = await axios.post(`${backendUrl}/api/product/review/remove`, { productId: selectedProduct._id, reviewId: r._id }, { headers: { token } });
//                                 if (res.data.success) {
//                                   toast.success('Review deleted');
//                                   // update local state
//                                   setList(prev => prev.map(p => p._id === selectedProduct._id ? { ...p, reviews: p.reviews.filter(rr => rr._id !== r._id) } : p));
//                                   setSelectedProduct(prev => ({ ...prev, reviews: (prev.reviews||[]).filter(rr => rr._id !== r._id) }));
//                                 } else toast.error(res.data.message);
//                               } catch (err) {
//                                 console.error(err);
...
//     category: '',
//     subCategory: '',
//     images: Array(10).fill(null), // { url, file }
//     videos: Array(2).fill(null),  // { url, file } -> Added Videos
//     manufacturerDetails: '',
//     manufacturerLabel: 'Manufacturer Details',
//     variantGroups: []
//   });

//   const mainCategories = ['Protein', 'Creatine', 'BCAA', 'Mass Gainer', 'Pre Workout', 'Post Workout', 'Vitamins'];
//   const subCategories = ['Popular', 'Just Launched', "Editor's Choice", 'Trending'];
...
//     // Prepare Images
//     const initialImages = Array(10).fill(null);
//     if(product.image && Array.isArray(product.image)) {
//         product.image.forEach((url, i) => {
//             if(i < 10 && url) initialImages[i] = { url, file: null };
//         });
//     }

//     // Prepare Videos
//     const initialVideos = Array(2).fill(null);
//     if(product.videos && Array.isArray(product.videos)) {
//         product.videos.forEach((url, i) => {
//             if(i < 2 && url) initialVideos[i] = { url, file: null };
//         });
//     }
...
//       // Handle Videos
//       formData.videos.forEach((vidObj, idx) => {
//         if (vidObj && vidObj.file) {
//             updateData.append(`video${idx + 1}`, vidObj.file);
//         } else if (vidObj === null) {
//             updateData.append(`removeVideo${idx + 1}`, 'true');
//         }
//       });
...
//               const updatedProduct = res.data.product || {};
//               const updatedImages = Array.isArray(updatedProduct.image) ? updatedProduct.image : item.image;
//               const updatedVideos = Array.isArray(updatedProduct.videos) ? updatedProduct.videos : item.videos; // Sync videos
//               return {
//                 ...item,
//                 name: formData.name,
//                 description: formData.description,
...
//         newVideos[idx] = { url: URL.createObjectURL(file), file };
//         return { ...prev, videos: newVideos };
//     });
//   };

//   const handleVideoRemove = (idx) => {
//     setFormData(prev => {
//         const newVideos = [...prev.videos];
//         newVideos[idx] = null;
//         return { ...prev, videos: newVideos };
...
//   // Deep copy helper to avoid state mutation bugs
//   const updateVariantGroup = (groupIndex, variantIndex, field, value) => {
//     setFormData(prev => {
//         // Deep copy the variant groups array
//         const newGroups = prev.variantGroups.map((group, gi) => {
//             if (gi !== groupIndex) return group;
//             return {
//                 ...group,
//                 variants: group.variants.map((variant, vi) => {
...
//                   <>
//                     <span className="line-through text-gray-500">{currency}{item.price}</span>{' '}
//                     <span className="text-blue-600 font-semibold">{currency}{Math.round(item.price - (item.price * item.discount / 100))}</span>
//                   </>
//                 ) : (
//                   <span>{currency}{item.price}</span>
//                 )}
//               </p>
//               <button onClick={() => removeProduct(item._id)} className="text-[#052659] border border-[#052659] px-2 py-1 text-xs rounded hover:bg-[#052659] hover:text-white transition">Remove</button>
//               <button onClick={() => handleEditClick(item)} className="text-blue-600 border border-blue-600 px-2 py-1 text-xs rounded hover:bg-blue-600 hover:text-white transition">Edit</button>
//             </div>
//           ))
//         ) : (
//           <div className="text-center py-8 text-gray-500">{searchQuery ? 'No products found' : 'No products available'}</div>
//         )}
//       </div>
...
//                   <div key={idx} className="relative">
//                     {imgObj ? (
//                       <img className="w-20 h-20 object-cover border rounded" src={imgObj.url} alt="" />
//                     ) : (
//                       <div className="w-20 h-20 flex items-center justify-center border rounded text-xs text-gray-500">Image {idx+1}</div>
//                     )}
//                     <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => { if (e.target.files[0]) handleImageChange(idx, e.target.files[0]); }} />
//                     {(imgObj && imgObj.url) && (
//                       <button type="button" className="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs" onClick={() => handleImageRemove(idx)}>×</button>
//                     )}
//                   </div>
//                 ))}
//               </div>

//               {/* Videos UI (ADDED THIS SECTION) */}
//               <p className="text-sm font-medium text-gray-700 mt-2">Product Videos</p>
//               <div className="flex gap-4">
//                 {formData.videos.map((vidObj, idx) => (
//                     <div key={idx} className="relative">
//                         {vidObj ? (
//                             <video className="w-24 h-24 object-cover border rounded bg-black" src={vidObj.url} controls={false} />
//                         ) : (
//                             <div className="w-24 h-24 flex items-center justify-center border rounded text-xs text-gray-500">Video {idx+1}</div>
//                         )}
//                         <input type="file" accept="video/*" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => { if (e.target.files[0]) handleVideoChange(idx, e.target.files[0]); }} />
//                         {(vidObj && vidObj.url) && (
//                             <button type="button" className="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-xs z-10" onClick={() => handleVideoRemove(idx)}>×</button>
//                         )}
//                     </div>
//                 ))}
//               </div>
...
//               {/* Name & Basic Info */}
//               <label className="text-sm font-medium text-gray-700 mt-2">Name:</label>
//               <input type="text" className="border px-3 py-2 rounded" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />

//               <label className="text-sm font-medium text-gray-700">Description:</label>
//               <textarea className="border px-3 py-2 rounded" rows={3} value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} />
              
//               <label className="text-sm font-medium text-gray-700 mt-2">Manufacturer Label:</label>
//               <input className="border px-3 py-2 rounded w-full max-w-sm" value={formData.manufacturerLabel} onChange={e => setFormData({ ...formData, manufacturerLabel: e.target.value })} />
              
//               <label className="text-sm font-medium text-gray-700 mt-2">Manufacturer Details:</label>
//               <textarea className="border px-3 py-2 rounded" rows={2} value={formData.manufacturerDetails} onChange={e => setFormData({ ...formData, manufacturerDetails: e.target.value })} />

//               {/* Price & Category */}
//               <div className="grid grid-cols-2 gap-4">
//                   <div>
//                     <label className="text-sm font-medium text-gray-700">Main Category:</label>
//                     <select className="border px-3 py-2 rounded w-full" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}>
//                         <option value="">Select Category</option>
//                         {mainCategories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
//                     </select>
//                   </div>
//                   <div>
//                     <label className="text-sm font-medium text-gray-700">Subcategory:</label>
//                     <select className="border px-3 py-2 rounded w-full" value={formData.subCategory} onChange={e => setFormData({ ...formData, subCategory: e.target.value })}>
//                         <option value="">Select Subcategory</option>
//                         {subCategories.map(subCat => <option key={subCat} value={subCat}>{subCat}</option>)}
//                     </select>
//                   </div>
...
//                   <div>
//                     <label className="text-sm font-medium text-gray-700">Price:</label>
//                     <input type="number" className="border px-3 py-2 rounded w-full" value={formData.price} onChange={e => setFormData({ ...formData, price: e.target.value })} />
//                   </div>
//                   <div>
//                     <label className="text-sm font-medium text-gray-700">Discount %:</label>
//                     <input type="number" min="0" max="100" className="border px-3 py-2 rounded w-full" value={formData.discount} onChange={e => setFormData({ ...formData, discount: e.target.valueAsNumber || 0 })} />
//                   </div>
//               </div>

//               {/* Variants */}
//               <div className="min-w-0 border-t pt-4">
//                 <p className="text-sm font-medium text-gray-700 mb-2">Variant Groups</p>
//                 <div className="space-y-3">
//                   {(formData.variantGroups || []).map((g, gi) => (
//                     <div key={gi} className="p-3 border border-gray-200 rounded min-w-0">
...
//                                 newGroups[gi].label = e.target.value;
...
//                             setFormData({...formData, variantGroups: newGroups});
//                         }} className="px-2 py-1 bg-red-600 text-white rounded text-xs">Remove Group</button>
//                       </div>

//                       <div className="mb-2">
//                         <button type="button" onClick={() => {
//                             const newGroups = [...formData.variantGroups];
//                             newGroups[gi].variants.push({ name: '', price: '', stock: '', imageIndex: '', discount: '' });
//                             setFormData({...formData, variantGroups: newGroups});
//                         }} className="px-2 py-1 bg-blue-600 text-white rounded text-xs">➕ Add Variant</button>

//                         <div className="space-y-2 mt-2">
//                           {(g.variants || []).map((vv, vi) => (
//                             <div key={vi} className="flex flex-col md:flex-row gap-2 items-center min-w-0 bg-gray-50 p-2 rounded">
//                               <input type="text" placeholder="Name" value={vv.name} onChange={(e) => updateVariantGroup(gi, vi, 'name', e.target.value)} className="px-2 py-1 border rounded w-full md:w-32" />
//                               <input type="number" placeholder="Price" value={vv.price} onChange={(e) => updateVariantGroup(gi, vi, 'price', e.target.value)} className="px-2 py-1 border rounded w-full md:w-24" />
//                               <input type="number" placeholder="Stock" value={vv.stock} onChange={(e) => updateVariantGroup(gi, vi, 'stock', e.target.value)} className="px-2 py-1 border rounded w-full md:w-20" />
                              
//                               {/* Image Selector for Variants */}
//                               <select value={vv.imageIndex || ''} onChange={(e) => updateVariantGroup(gi, vi, 'imageIndex', e.target.value)} className="px-2 py-1 border rounded w-full md:w-32">
//                                 <option value="">Default Img</option>
//                                 {[...Array(10)].map((_, i) => <option key={i} value={i+1}>Img {i+1}</option>)}
//                               </select>

//                               <button type="button" onClick={() => {
//                                 const newGroups = [...formData.variantGroups];
//                                 newGroups[gi].variants.splice(vi, 1);
//                                 setFormData({...formData, variantGroups: newGroups});
//                               }} className="px-2 py-1 bg-red-600 text-white rounded text-xs">Del</button>
//                             </div>
//                           ))}
//                         </div>
//                       </div>
//                     </div>
//                   ))}
//                   <button type="button" onClick={() => setFormData(prev => ({ ...prev, variantGroups: [...prev.variantGroups, { label: 'New Group', variants: [] }] }))} className="mt-2 px-3 py-1 bg-green-600 text-white rounded text-sm">Add New Group</button>
//                 </div>
//               </div>
...
//   // DEEP COPY HELPER for Variants (Fixes state mutation bugs)
//   const updateVariantGroup = (groupIndex, variantIndex, field, value) => {
//     setFormData(prev => {
//         // Deep copy the variant groups array
//         const newGroups = prev.variantGroups.map((group, gi) => {
...
const List = ({ token }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    subCategory: '',
    images: Array(10).fill(null), // { url, file }
    videos: Array(2).fill(null),  // { url, file }
    manufacturerDetails: '',
    manufacturerLabel: 'Manufacturer Details',
    variantGroups: []
  });
  ...
  const handleEditClick = (product) => {
    ...
    setFormData({
      ...
      category: product.category,
      subCategory: product.subCategory,
      // Default to one empty group if none exists, or use existing
      variantGroups: product.variantGroups && product.variantGroups.length > 0 
        ? product.variantGroups 
        : [{ label: 'Flavour', variants: [] }],
      images: initialImages,
      videos: initialVideos,
      manufacturerDetails: product.manufacturerDetails || '',
      manufacturerLabel: product.manufacturerLabel || 'Manufacturer Details'
        ...
        setList(prevList =>
          prevList.map(item => {
            if (item._id === selectedProduct._id) {
              const updatedProduct = res.data.product || {};
              const updatedImages = Array.isArray(updatedProduct.image) ? updatedProduct.image : item.image;
              const updatedVideos = Array.isArray(updatedProduct.videos) ? updatedProduct.videos : item.videos;
              return {
                ...item,
                name: formData.name,
                description: formData.description,
                          ...
                          {(g.variants || []).map((vv, vi) => (
                            <div key={vi} className="flex flex-col md:flex-row gap-2 items-center min-w-0 bg-gray-50 p-2 rounded">
                              <input type="text" placeholder="Name" 
                                value={vv.name || ''} 
                                onChange={(e) => updateVariantGroup(gi, vi, 'name', e.target.value)} 
                                className="px-2 py-1 border rounded w-full md:w-32" 
                              />
                              <input type="number" placeholder="Price" 
                                value={vv.price !== undefined ? vv.price : ''} 
                                onChange={(e) => updateVariantGroup(gi, vi, 'price', e.target.value)} 
                                className="px-2 py-1 border rounded w-full md:w-24" 
                              />
                              <input type="number" placeholder="Stock" 
                                value={vv.stock !== undefined ? vv.stock : ''} 
                                onChange={(e) => updateVariantGroup(gi, vi, 'stock', e.target.value)} 
                                className="px-2 py-1 border rounded w-full md:w-20" 
                              />
                              
                              {/* Image Selector for Variants */}
                              <select 
                                value={vv.imageIndex || ''} 
                                onChange={(e) => updateVariantGroup(gi, vi, 'imageIndex', e.target.value)} 
                                className="px-2 py-1 border rounded w-full md:w-32"
                              >
                                <option value="">Default Img</option>
                                {[...Array(10)].map((_, i) => <option key={i} value={i+1}>Img {i+1}</option>)}
