...
const Login = ({ setToken }) => {
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(base + '/api/user/admin', payload);
    ...
    const handleSendForgotOtp = async () => {
        ...
        try {
            const base = backendUrl || 'http://localhost:4000';
            const res = await axios.post(base + '/api/user/admin/forgot-password', { email, adminCode }, { timeout: 20000 });
            if (res.data.success) {
    ...
    const handleVerifyForgotOtpAndReset = async () => {
        ...
        try {
            const base = backendUrl || 'http://localhost:4000';
            const verifyRes = await axios.post(base + '/api/user/admin/verify-forgot-otp', { email, otp, adminCode }, { timeout: 20000 });
    ...
    const handleVerifyLoginOtp = async () => {
        ...
        try {
            const base = backendUrl || 'http://localhost:4000';
            const verifyRes = await axios.post(base + '/api/user/verify-login-otp', { email, otp }, { timeout: 20000 });
            if (!verifyRes.data.success) return toast.error(verifyRes.data.message || 'OTP verification failed');
    ...
    const handleSendOTP = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                base + '/api/user/admin/change-password/send-otp',
                { currentPassword },
                { 
                    headers: { 
                        token: token 
    ...
    const handleVerifyOTPAndChangePassword = async () => {
        ...
        try {
            const base = backendUrl || 'http://localhost:4000';
            const response = await axios.post(
                base + '/api/user/admin/change-password/verify',
                {
                    currentPassword,
                    newPassword,
                    confirmPassword,
                    ...
                    forgotPasswordMode ? (
                        ...
                        <div>
                            {forgotStep === 1 && (
                                <div>
                                    <div className='mb-4'>
                                        <p className='text-sm font-medium text-gray-700 mb-2'>Admin Code </p>
                                        <input
